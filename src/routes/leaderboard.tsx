...
function LeaderboardPage() {
  ...
  const list = tab === "tab_friends" ? PLAYERS.slice(0, 3) : tab === "tab_weekly" ? PLAYERS.slice(2) : PLAYERS;
        ...
        {TABS.map((tb) => (
          <button
            ...
            className={`flex-1 rounded-xl px-2 py-2 text-xs font-semibold transition-all duration-300 ${
              tab === tb ? "bg-gold/20 text-gold-dark" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {t(tb)}
          </button>
